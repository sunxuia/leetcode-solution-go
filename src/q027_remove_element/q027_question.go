...
func doTest(t *testing.T, method func([]int, int) int) {
	...
	var res int
	start, end := 0, 0
	cfg := test.NewAssertConfig().AddValidator("", test.GetAssertArrayRange(&start, &end))
	...
	th.Expect([]int{2, 2}).Config(cfg).Assert(arr)

	defer th.NewTestCase()()
	arr = []int{0, 1, 2, 2, 3, 0, 4, 2}
	res = method(arr, 2)
	th.Expect(5).Assert(res)
	end = res - 1
	th.Expect([]int{0, 1, 3, 0, 4}).Config(cfg).Assert(arr)
